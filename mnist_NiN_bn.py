...
def inference(x, phase=False, keep_prob=1.0):
    ...
    with tf.compat.v1.variable_scope('mlp1-1'):
        x = conv(x, phase, [1, 1, 192, 160])
        x = activation(x)

    with tf.compat.v1.variable_scope('mlp1-2'):
        x = conv(x, phase, [1, 1, 160, 96])
        x = activation(x)

    with tf.compat.v1.name_scope('max_pool-1'):
        x  = max_pool(x, 3, 2)
    ...
    with tf.compat.v1.variable_scope('mlp2-1'):
        x = conv(x, phase, [1, 1, 192, 192])
        x = activation(x)

    with tf.compat.v1.variable_scope('mlp2-2'):
        x = conv(x, phase, [1, 1, 192, 192])
        x = activation(x)

    with tf.compat.v1.name_scope('max_pool-2'):
        x  = max_pool(x, 3, 2)
    ...
    with tf.compat.v1.variable_scope('mlp3-1'):
        x = conv(x, phase, [1, 1, 192, 192])
        x = activation(x)

    with tf.compat.v1.variable_scope('mlp3-2'):
        x = conv(x, phase, [1, 1, 192, 10])
        x = activation(x)

    with tf.compat.v1.name_scope('global_avg_pool'):
